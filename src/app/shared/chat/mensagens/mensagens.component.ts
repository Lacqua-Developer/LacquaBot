...
export class HistoricoMensagemComponent implements OnInit, OnDestroy {
  ...
  AbreSessao(): void {
    ...
    // Se Sessao estiver indefinido, cria uma nova instância
    if (this.contatoSelecao.Sessao?.IdSession == null) {
      this.contatoSelecao.Sessao = new Sessao();
    }

    // Criando uma cópia profunda de contatoSelecao, incluindo Sessao
    let contatoSelecaoLocal = {
      ...this.contatoSelecao,
      Sessao: {
        ...this.contatoSelecao.Sessao, // Cópia profunda do objeto Sessao
      },
    };

    // Agora é seguro alterar a propriedade da cópia
    contatoSelecaoLocal.Sessao.Usuario_IdUsuario = this.usr.IdUsr;

    // Abrindo o diálogo com a cópia local de contatoSelecao
    const dialogRef = this.dialog.open(AbreAtendimentoComponent, {
      width: '350px',
      height: '250px',
      data: {
        contato: contatoSelecaoLocal, // Passando a cópia ao diálogo
        usuarios: this.usr,
    ...
    if (this.TipoChat == '3') {
      dialogRef.afterClosed().subscribe((result) => {
        ...
        // Resetando valores após o fechamento do diálogo
        this.contato = new Contato();
        this.contatoSelecao = new Contato();
        this.ConversaSelecao = [];
        this.currentsessao = 0;
    ...
    // Verificando listas imediatamente após abrir o diálogo
    this.core.verificaEspera();
    this.core.verificaAtivos(this.usr.IdUsr.toString());
    this.core.verificaContatos(
      this.usr.IdUsr.toString(),
